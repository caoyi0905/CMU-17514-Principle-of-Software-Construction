...
//        return result;
    ...
    public void renderArticles(Project project) throws IOException {
        ...
        for (Article article : project.getArticles()) {
            ...
            recursiverenderArticle(project, article);
        }
    }


    public void recursiverenderArticle  (Project project, Article article) throws IOException {

        if (article.getInnerArticles() == null)
            return ;
        for(Article subarticle: article.getInnerArticles()){
            renderArticle(project,article);
            recursiverenderArticle(project,subarticle);
        }
    ...
    public List<SiteLink> getBreadcrumbs(Article article) {
        if (article.getParent() == null){
            ...
            if(article.getInnerArticles() == null){
                ...
                result.add(0,new SiteLink(getArticleURL(article.getParent()), article.getParent().getTitle(), true));
                result.add(0,new SiteLink(getArticleURL(article.getParent().getParent()), article.getParent().getParent().getTitle(), true));
            }else{
                result.add(new SiteLink(getArticleURL(article), article.getTitle(), true));
                result.add(0,new SiteLink(getArticleURL(article.getParent()), article.getParent().getTitle(), true));
            }
...
//        templateEngine.render(page.getTemplate(), page, targetFile);
    ...
    public ContentFragment getSubArticleFragment(Article subarticle, String relPath) throws IOException {
        StringWriter w = new StringWriter();
        this.templateEngine.render("article-preview",
                renderArticlePreview(subarticle, relPath, "Read on: "), w);
        return new ContentFragment(subarticle.getTitle(), w.toString());
    }


//    public ContentFragment getSubSubArticleFragment(Article subsubarticle, String relPath) throws IOException {
//        StringWriter w = new StringWriter();
//        this.templateEngine.render("article-preview",
//                renderArticlePreview(subsubarticle, relPath, "Read on: "), w);
...
//    }
    ...
    public List<ContentFragment> getArticleContent(Article story, String relPath) throws IOException {
        ...
        for (Article n : story.getInnerArticles())
...
//    public List<ContentFragment> getSubArticleContent(Article story, String relPath) throws IOException {
//        List<ContentFragment> result = new ArrayList<>();
//        for (AbstractContent n : story.getContent())
//            result.add(getStoryContentFragment(n, relPath));
//        for (Article n : story.getInnerArticles())
...
//    }
//
//    public List<ContentFragment> getSubSubArticleContent(Article story, String relPath) throws IOException {
//        List<ContentFragment> result = new ArrayList<>();
//        for (AbstractContent n : story.getContent())
//            result.add(getStoryContentFragment(n, relPath));
////        for (Event n : story.getInnerEvents())
...
//    }
    ...
    public List<Object> findAllArticles(Project project) {
        ...
        for (Article article : project.getArticles()) {
            recursivefindAllArticles(project,article,result);
        }
        return result;
    }

    public void recursivefindAllArticles (Project project, Article article, List<Object> result){
        if (article.getInnerArticles() == null){
            return ;
        }

        for (Article subarticle : article.getInnerArticles()){
            result.add(subarticle);
            recursivefindAllArticles(project,subarticle,result);
        }
    }
    ...
    public List<Object> findArticlesByTopic(Project project, Topic topic) {
        List<Object> result = new ArrayList<>();
        for(Article article : project.getArticles()){
            recursivefindArticleByTopic(project,topic,article,result);
        }
        return result;
    }
...
//        }
    ...
    public void recursivefindArticleByTopic (Project project, Topic topic, Article article, List<Object> result){
        if (article.getInnerArticles() == null){
            return ;
        }
        for (Article subarticle : article.getInnerArticles() ){
            if (project.getTopics(subarticle).contains(topic))
                result.add(subarticle);
            recursivefindArticleByTopic(project,topic,subarticle,result);
        }
    }

    public void renderTopic(Project project, Topic topic) throws IOException {
        List<Object> allArticles = this.findArticlesByTopic(project, topic);
...
//    public SiteURL getSubArticleURL(Article entry) {
//        return createURL(getSubArticlePath(entry));
//    }
//
//    public SiteURL getSubSubArticleURL(Article entry) {
...
//    }
    ...
    public String getSubArticlePath(Article article) {
        String path = article.getId() + "/";
    ...
    public String getSubSubArticlePath(Article article) {
        String path = article.getId() + "/";
        Article parent = article.getParent();
        if (parent != null) {
            path = parent.getId() + "/" + path;
    ...
    public void renderArticleList(Project project) throws IOException {
        ...
        for (int pageIdx = 0; pageIdx < articlePages.size(); pageIdx++) {
            ...
            for (Object s : articles) {
                if (s instanceof Article a)
                    previews.add(renderArticlePreview(a, relPath, ""));
//                if (s instanceof Article sa)
//                    previews.add(renderArticlePreview(sa, relPath, ""));
//                if (s instanceof Article ssa)
//                    previews.add(renderArticlePreview(ssa, relPath, ""));
            }
...
//    public ArticlePreview renderSubArticlePreview(Article article, String relPath, String prefix) {
//        StringWriter w = new StringWriter();
//        int previewLength = 200;
//        for (AbstractContent c : article.getContent()) {
//            if (c instanceof FormattedTextDocument) {
...
//    public ArticlePreview renderSubSubArticlePreview(Article article, String relPath, String prefix) {
//        StringWriter w = new StringWriter();
//        int previewLength = 200;
//        for (AbstractContent c : article.getContent()) {
//            if (c instanceof FormattedTextDocument) {
