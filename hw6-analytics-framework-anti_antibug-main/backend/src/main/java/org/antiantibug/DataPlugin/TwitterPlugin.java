...
public class TwitterPlugin implements DataPlugin {
    ...
    private static final String[] PARAM_TEXT = new String[]{"Username", "Start-Time(yyyy-MM-dd HH:mm:ss)",
    ...
    public String getTitle() {
        ...
        title = "Tweets of user: " + user.getName();
        return title;
    }
    ...
    public Map<String, Object> provideData(Map<String, String> params) {
        ...
        this.userName = params.get(PARAM_TEXT[0]);
    ...
    private String getUserId() {
        user = twitterClient.getUserFromUserName(userName);
        return user.getId();
    }

    private TweetList getUserTimeline() {
        TweetList list = twitterClient.getUserTimeline(userId, additionalParameters);
        if (list == null) {
    ...
    private void getCreatedTime() {
        for (Tweet tweet : tweetList.getData()) {
            long longTimeStamp = tweet.getCreatedAt().toInstant(ZoneOffset.ofHours(17)).toEpochMilli();
    ...
    public static void main(String[] args) {
        TwitterPlugin twitterPlugin = new TwitterPlugin();
    }
}